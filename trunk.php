...
while($row=mysql_fetch_row($cmim))
	$operator[]=array($row[1], $row[2], $row[3], $row[4], $row[5]);
...
$result = mysql_query($query_src);
$tnr = 0;
//while ($row = mysql_fetch_row($result)) {
	$gtotal = 0;
	$totaltime = 0;
	$nrrend = 0;
	$result2=mysql_query($query_src_list);
?>
...
<?php
	...
	while($row = mysql_fetch_row($result2)) {
		...
		$total = round($row[2]*(($op[2]+$op[3])/60),2)+0.83;
...
$queries = array(
	'albtelekom' => "select count(*), sum(billsec) from cdr where (disposition='ANSWERED' and lastdata like 'DAHDI/G1%' and dst like '04%' or dst like '003554%' and src like '%4535%')".$datetime,
	'amc'        => "select count(*), sum(billsec) from cdr where (disposition='ANSWERED' and lastdata like 'DAHDI/G1%' and dst like '068%' or dst like '0035568%' and src like '%4535%')".$datetime,
	'vodafone'   => "select count(*), sum(billsec) from cdr where (disposition='ANSWERED' and lastdata like 'DAHDI/G1%' and dst like '069%' or dst like '0035569%' and src like '%4535%')".$datetime,
	'plus'       => "select count(*), sum(billsec) from cdr where (disposition='ANSWERED' and lastdata like 'DAHDI/G1%' and dst like '066%' or dst like '0035566%' and src like '%4535%')".$datetime,
	'eagle'      => "select count(*), sum(billsec) from cdr where (disposition='ANSWERED' and lastdata like 'DAHDI/G1%' and dst like '067%' or dst like '0035567%' and src like '%4535%')".$datetime
);
...
foreach ($queries as $k=>$v) {
	...
	while ($row = mysql_fetch_row($result)) {
		...
		if($a[0]!="Albtelecom") {
			$fee += round(0.02*$row[1]*(($a[2]+$a[3])/60),2);
		}
		$total += round($row[1]*(($a[2]+$a[3])/60) + ($row[0]*0.83),2);
		print "<td align=right><b>".round($row[1]*(($a[2]+$a[3])/60) + ($row[0]*0.83),2)."</b></td>";
		print "</tr>";
		}
}
print "</table><table width=\"500\" border=0 cellpadding=2 cellspacing=2 style=\"border-top:1px solid #000;\">";
print "<tr><td></td><td></td><td align=right><b>Totali :</b></td><td align=right><b>$total</b></td></tr>";
print "<tr><td></td><td></td><td align=right><b>Tarifimi 2% :</b></td><td align=right><b>$fee</b></td></tr>";
print "<tr><td></td><td></td><td align=right><b>G. Total :</b></td><td align=right><b>".round($total+$fee,2)."</b></td></tr>";
print "</table></div>";
